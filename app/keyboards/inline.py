...
from app.services.promo import get_active_discount, get_effective_tariffs
...
async def payment_keyboard(payment_type: str = "stars", balance: int | None = None, user_id: int | None = None) -> InlineKeyboardMarkup:

    keyboard = []
    tariffs = await get_effective_tariffs(user_id) if user_id is not None else await get_tariffs()

    # Если у пользователя сейчас есть неиспользованная скидка от
    # промокода — показываем это отдельной строкой сверху, чтобы было
    # видно, что цены ниже уже с её учётом (а не "прибавляется" ещё раз
    # где-то на оплате).
    if user_id is not None:
        active_discount = await get_active_discount(user_id)
        if active_discount:
            keyboard.append([
                InlineKeyboardButton(
                    text=f"🎟 Промо-скидка {active_discount}% уже применена ниже",
                    callback_data="noop",
                )
            ])
