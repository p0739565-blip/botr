...
def parse_client_config(raw_text: str) -> list[ParsedLink]:
    ...
    # Название присваиваем только теперь, когда точно знаем, сколько
    # РЕАЛЬНЫХ серверов нашлось — freedom/blackhole не должны
    # заставлять единственный сервер получать суффикс с тегом.
    multiple = len(results) > 1
    final_results: list[ParsedLink] = []
    for parsed, tag in results:
        if base_name and multiple:
            name = f"{base_name} — {tag}"
        elif base_name:
            name = base_name
        else:
            name = tag
        final_results.append(ParsedLink(url=parsed, name=name, tag=tag))
