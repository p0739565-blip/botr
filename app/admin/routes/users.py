...
from sqlalchemy import String, delete, exists, func, select
...
# Тот же порог "скоро истекает", что и на дашборде (см.
# app.admin.routes.dashboard.EXPIRING_SOON_WINDOW) — чтобы фильтр
# "Истекают скоро" на списке пользователей и виджет на дашборде
# показывали согласованные цифры, а не два разных определения "скоро".
...
async def users_list(
    ...
    expiring_cutoff = now + EXPIRING_SOON_WINDOW
    page = max(1, page)

    # EXISTS-подзапросы вместо загрузки всех подписок в Python — так
    # фильтр по статусу и пагинация применяются на уровне SQL, а не
    # "долистыванием" уже выгруженных 200 строк, как было раньше.
    ...
    is_expiring_soon = exists(
        select(Subscription.id).where(
            Subscription.user_id == User.id,
            Subscription.is_active == True,  # noqa: E712
            Subscription.expiry > now,
            Subscription.expiry <= expiring_cutoff,
        )
    )
    ...
    async with async_session() as session:
        ...
        if status == "active":
            stmt = stmt.where(has_active_sub)
        elif status == "none":
            stmt = stmt.where(~has_active_sub)
        elif status == "expiring":
            stmt = stmt.where(is_expiring_soon)
        else:
            status = "all"

        count_stmt = select(func.count()).select_from(stmt.subquery())
        total_filtered = (await session.execute(count_stmt)).scalar_one()

        total_pages = max(1, (total_filtered + USERS_PAGE_SIZE - 1) // USERS_PAGE_SIZE)
        page = min(page, total_pages)
        offset = (page - 1) * USERS_PAGE_SIZE

        stmt = (
            stmt.options(selectinload(User.subscriptions))
        ...
        # ---- Карточки статистики (всегда по ВСЕЙ базе, без учёта
        # текущего поиска/фильтра/страницы — так же, как на дашборде) ----
        total_users = (
            await session.execute(select(func.count(User.id)))
        ).scalar_one()

        active_subs_total = (
            await session.execute(select(func.count(User.id)).where(has_active_sub))
        ).scalar_one()

        expiring_soon_total = (
            await session.execute(select(func.count(User.id)).where(is_expiring_soon))
        ).scalar_one()

        new_users_week = (
            await session.execute(
                select(func.count(User.id)).where(
                    User.created_at >= now - datetime.timedelta(days=7)
                )
            )
        ).scalar_one()
    ...
    no_sub_total = total_users - active_subs_total
    active_pct = round(active_subs_total / total_users * 100, 1) if total_users else 0
