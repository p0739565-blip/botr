...
from app.services.promo import consume_discount, get_effective_tariff, get_effective_tariffs
...
async def choose_stars_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "💎 Выберите тариф💎 :",
        reply_markup=await payment_keyboard("stars", user_id=user.id),
    )
...
async def choose_card_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "💎 Выберите тариф💎 :",
        reply_markup=await payment_keyboard("card", user_id=user.id),
    )
...
async def choose_sbp_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "💎 Выберите тариф💎 :",
        reply_markup=await payment_keyboard("sbp", user_id=user.id),
    )
...
async def choose_crypto_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "💎 Выберите тариф💎 :",
        reply_markup=await payment_keyboard("crypto", user_id=user.id),
    )
...
async def choose_balance_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=await payment_keyboard("balance", balance=user.balance, user_id=user.id),
    )
...
async def buy_stars(callback: CallbackQuery):
    ...
    tariff = await get_effective_tariff(tariff_key, user.id)
...
async def buy_balance(callback: CallbackQuery):
    ...
    # Скидка (если была зарезервирована промокодом) относится к ОДНОЙ
    # покупке — гасим её сразу после того, как эта покупка состоялась,
    # чтобы не применилась повторно к следующей.
    await consume_discount(user.id)
