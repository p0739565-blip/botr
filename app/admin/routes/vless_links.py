...
from app.admin.config_json_import import ConfigJsonError, parse_client_config
...
async def _groups_for(session, is_dead: bool) -> list[VlessLinkGroup]:
    ...
    # relationship VlessLinkGroup.links не имеет order_by, поэтому
    # сортируем здесь явно — иначе кнопки "выше"/"ниже" меняют
    # position в БД, но список в админке продолжает показывать
    # старый порядок.
    for group in groups:
...
async def create_link(
    ...
    """Создаёт ссылку(и) одним из двух способов:

    - `url` — как раньше, готовая ссылка-конфиг (vless://, hy2:// и т.п.);
    - `config_json` — вставленный целиком клиентский JSON-конфиг Xray
      (с разделом "outbounds"). Из него автоматически достаются все
      реальные сервера (может быть больше одного — например, если в
      конфиге balancer из нескольких серверов) и превращаются в такие
      же ссылки, как при ручном вводе.

    Оба поля разом не ожидаются — если заполнен config_json, url
    игнорируется.
    """
    from app.admin.router import templates
    ...
    # ---- ветка: вставлен JSON-конфиг целиком ----
    if config_json:
        try:
            parsed_links = parse_client_config(config_json)
        except ConfigJsonError as exc:
            return await _render_error(str(exc))
        ...
        async with async_session() as session:
            ...
            for parsed in parsed_links:
                # Если админ явно задал своё имя, а сервер в JSON один —
                # используем его; иначе берём имя, которое собрал парсер
                # (remarks/tag из конфига — так разные сервера из одного
                # balancer не получат одинаковое имя).
                link_name = name.strip() if (name.strip() and len(parsed_links) == 1) else parsed.name
                final_url = _apply_name(parsed.url, link_name)
        ...
        await write_audit_log(
            admin,
            action="create_vless_link_from_json",
            target=f"vless_link_ids={created_ids}",
            details=f"is_dead={is_dead_bool}, count={len(created_ids)}",
        )

        return RedirectResponse("/admin/vless-links", status_code=303)

    # ---- ветка: как раньше, готовая ссылка одной строкой ----
    if not url:
        return await _render_error("Заполните либо ссылку-конфиг, либо JSON-конфиг")

    if not url.startswith(LINK_PREFIXES):
        return await _render_error(
            "Похоже, это не ссылка-конфиг (ожидается vless://, hy2:// и т.п.)"
        )
