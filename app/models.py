...
class PromoCode(Base):
    """Промокод. Может давать один из двух эффектов или оба сразу:

    - discount_percent — скидка в % на СЛЕДУЮЩУЮ покупку подписки
      (любым способом оплаты). Скидка не списывается сразу при вводе
      кода, а "резервируется" за пользователем (см. PromoRedemption) —
      применяется автоматически на ближайшей покупке и гасится после
      неё. Одновременно активна только одна зарезервированная скидка
      на пользователя (см. app.services.promo).
    - bonus_balance — фиксированная сумма в рублях, начисляется на
      бонусный баланс пользователя СРАЗУ при активации кода, как
      ручное начисление админом (см. grant_balance), только без
      участия админа.

    code хранится в верхнем регистре — сравнение и ввод пользователем
    регистронезависимы (см. app.services.promo.redeem_code).

    max_activations=None — без лимита. activations_count — кэш-счётчик
    для быстрого отображения в списке в админке; источник истины по
    факту активаций — таблица PromoRedemption (см. get_promo_stats)."""
    ...
    # Заметка для админа (например "промо на 23 февраля в паблике") —
    ...
    def is_expired(self) -> bool:
        return self.expires_at is not None and self.expires_at <= datetime.datetime.now()

    @property
    def is_exhausted(self) -> bool:
        return self.max_activations is not None and self.activations_count >= self.max_activations
...
class PromoRedemption(Base):
    """Одна активация промокода одним пользователем. Уникальная пара
    (promo_code_id, user_id) — один и тот же код нельзя активировать
    дважды одному пользователю, даже если у кода ещё остались свободные
    активации.

    discount_percent_reserved — скопированная на момент активации
    скидка кода (история не "уезжает", если код потом изменят/удалят).
    discount_used_at — когда скидка была списана на реальной покупке;
    NULL — скидка ещё ждёт применения. Для кодов без discount_percent
    оба поля остаются NULL с самого начала."""

    __tablename__ = "promo_redemptions"

    id: Mapped[int] = mapped_column(primary_key=True)
    promo_code_id: Mapped[int] = mapped_column(
        ForeignKey("promo_codes.id"), index=True
    )
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)

    bonus_balance_granted: Mapped[int | None] = mapped_column(Integer, nullable=True)

    discount_percent_reserved: Mapped[int | None] = mapped_column(
        Integer, nullable=True
    )
    discount_used_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime(), nullable=True
    )
